function dydt = trj_eqs_gt(t,y,RktSpc,CL,CD,t_tot)
...
dydt = zeros(4,1);
%du/dt
dydt(1) = ((T/(m_t)))  - ( (CD_t/(2*m_t))*(rho*y(1)^2*area) ) - ( ((g_dot*y(4))+g_0)*sin(y(2)) );
%dtheta/dt
dydt(2) =  ( (CL_t/(2*m_t))*(rho*y(1)*area) ) - (((g_dot*y(4))+g_0*cos(y(2)))./y(1));
%dX/dt
dydt(3) = y(1)*cos(y(2));
%dh/dt
dydt(4) = y(1)*sin(y(2));
